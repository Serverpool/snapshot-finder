...
parser.add_argument('-b', '--blacklist', default='', type=str,
                    help='If the same corrupted archive is constantly downloaded, you can exclude it.'
...
def get_snapshot_slot(rpc_address: str):
    ...
    try:
        ...
        if 'location' in str(r.headers) and 'error' not in str(r.text):
            ...
            if slots_diff < -100:
                logger.error(
                    f'Something wrong with this snapshot\\rpc_node - {slots_diff=}. This node will be skipped {rpc_address=}')
                DISCARDED_BY_SLOT += 1
                return
            ...
            if 'location' in str(r.headers) and 'error' not in str(r.text):
                json_data["rpc_nodes"].append({
                    ...
                    "cost": (
                                    AVERAGE_SNAPSHOT_FILE_SIZE_MB + AVERAGE_INCREMENT_FILE_SIZE_MB) / MIN_DOWNLOAD_SPEED_MB + slots_diff / AVERAGE_CATCHUP_SPEED
                })
                return
    ...
    try:
        if MAX_DOWNLOAD_SPEED_MB is not None:
            process = subprocess.run(
                ['/usr/bin/wget', f'--limit-rate={MAX_DOWNLOAD_SPEED_MB}M', '--trust-server-names', url,
                 f'-O{temp_fname}'],
...
def main_worker():
    try:
        ...
        if len(FULL_LOCAL_SNAPSHOTS) > 0:
            ...
            logger.info(
                f'Can\'t find any full local snapshots in this path {SNAPSHOT_PATH} --> the search will be carried out on full snapshots')
